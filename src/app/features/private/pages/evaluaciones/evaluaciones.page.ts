...
export class EvaluacionesPage implements OnInit {
    tabActiva = 0; // esto es una variable GLOBAL
    ...
    async generatePdf() {
        try {
            ...
            const club = document.querySelector('#staticBackdrop .borders-club h5')?.textContent?.trim() || 'Desconocido';
            const puntuacion = element.querySelector('.puntuacion h3')?.textContent?.trim() || 'N/A';
            const porcentaje = element.querySelector('.badge')?.textContent?.trim() || 'N/A';
            const evaluadorRaw = element.querySelector('.card-body small')?.textContent?.trim() || '';
            const evaluador = (evaluadorRaw.replace(/Nombre del evaluador:\s*/i, '').trim()) || 'Desconocido';
            const ps = Array.from(element.querySelectorAll('.card-body p'));
            const observaciones = (ps.length ? ps[ps.length - 1].textContent?.trim() : '') || 'Sin observaciones';
            ...
            const selectEl = document.querySelector('select.form-select') as HTMLSelectElement | null;
            const selectedClub = selectEl?.selectedOptions?.[0]?.text?.trim() || club;
            const body = cards.map(card => {
                const punt = card.querySelector('.puntuacion h3')?.textContent?.trim() || puntuacion;
                const porc = card.querySelector('.badge')?.textContent?.trim() || porcentaje;
                const evalRaw = card.querySelector('.card-body small')?.textContent?.trim() || '';
                const evalName = (evalRaw.replace(/Nombre del evaluador:\s*/i, '').trim()) || evaluador;
